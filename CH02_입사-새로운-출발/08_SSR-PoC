...
(※ 일을 할 때, 항상 목표를 세우고 그 범주 안에서 일을 해야한다. 그걸 하지 않으면, 막연하게 그리고 또 한없이 시간을 쓰고도 정작 길을 잃어버릴 수도 있는 일들이 굉장히 비일비재하다.)
...
- 페이지별 Meta Taging <br />
  > 가능한지. 잘 되는지. 확인. <br />
  > 왜냐하면 SEO를 우리가 분석을 해봤을 때, 기술적인 측면에선 여러 가지 중요한 것들이 굉장히 많지만 핵심적인 것은 이 컨텐츠를 검색엔진한테 적절하게 잘 설명해 주는 것. 그래서 메타 태그를 잘 세팅하는 것이 중요했기 때문에... :) (공유도 마찬가지)
- API Fetching <br />

  > 어플리케이션을 커다란 덩어리(!)로 구분지어 보자면 어떻게 구분지어 볼 수 있을까?<br /> `데이터`가 있고, 그 데이터를 `UI`로 만드는 로직이 있다. 그리고 클라이언트는 이름에서도 알 수 있듯이 스스로 데이터를 가지고 있지 않다. 그래서 대부분의 데이터를 remote에서 가져온다. 그렇지 않은 경우도 아주 가끔 있긴 하지만, 보통은 그 'remote'가 API 서버인 경우가 많고, 거기서 데이터를 가져오는 경우가 많다. 그렇기에 네트워크를 이용해서 데이터를 가져오는 `데이터 페칭(fetching)`이 굉장히 중요할 것이다. 데이터 fetching 로직... 여기엔 부수적으로 에러 처리나 예외 처리 등등 여러가지가 있다보니 데이터가 컸을 때 혹은 여러 가지의 API를 동시 다발적으로 호출해야 될 때와 같은 다양한 상황들에서 여러 가지 문제들이 생길 수 있으며, 또 '코드의 구조'를 어떻게 만드는지에 대한 고민들이 많이 들어가는 부분 가운데 하나다.<br /> > <br />
...
- Next.js 기능 목록 체크<br />
  ...
  > 지금 아주 특별한 서비스를 개발하는 것은 아니니까 이 정도 측면에서면 충분해보이고 그럼에도 불구하고 어떤 기능들이 있는지 나머지 것들도 한번 쑥 살펴볼 필요성들이 존재한다. 그 중에 생각지 못한 유용한 것들이 있을 수 있으니까 :) (오... 저거 써먹어야겠다! 싶은 부분들이 있을 수 있으니까)<br /> > <br />
  > 이와 같은 측면에서 기능 목록 체크를 한다는 의미... :)<br /> > <br />
  > 사실, 대부분의 개발자들이 공식 문서와 제공되는 기능 목록을 상세하게 잘 보지는 않는다. 다른 이유보다도 시간이 없기 때문...🥲🥲🥲 이런 부분을 상기시키고자 넣은 항목이고, 개발자로서 어떤 걸 먼저 봐야하고, 뭘 나중에 봐도 되는지를 잘 구분하고 분류하는 감각과 능력을 길러나가야 한다. (나중에 봐도 될거라고 생각한 부분에서 문제가 생길여지는 늘 있지만 🥲🥲🥲 그럼에도 불구하고 '의도적인 훈련'을 해야만 이런 능력들이 쌓여나갈 것이다.)
...
- [Client side](https://nextjs.org/docs/basic-features/data-fetching/client-side)<br />
  ...
  > ※ 근데 가만 생각해보면, Next.js는 SSR 프레임워크인데 굳이 CSR 데이터 페칭을 왜 다룰까? 문서 내용도 그렇고... (조금 더 내려보면 SWR이 있다. ㅋㅋ 😅)<br />

  문서를 보다가 새로운 게 (여기서는 SWR) 나와서 거기에 주의를 뺏기면 안된다. 적당히 살펴보고 RFC 참조 문서에 링크를 달기... ^^; 브레이크가. 있어야 한다. 🥲 그래야 일을 합리적으로 할 수 있다.
