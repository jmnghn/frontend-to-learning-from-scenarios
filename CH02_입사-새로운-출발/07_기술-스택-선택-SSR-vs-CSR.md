...
```
Q. "그 기술을 왜 선택했어요?"
A. "이 기술을 외부에서 굉장히 많이 쓰는 것 같드라구요~"
A. "여러 사람들이 이 기술이 좋다고 하더라구요~ 그래서 내가 안해봤는데 이걸 한번 해보고 싶었어요. 
그래서 이번에 프로젝트를 하는 김에 이 기술을 한번 선택해봤어요~"
혹은 "이런 프로젝트 유형은 이 기술로 하는 것 아닌가요?"
...😣<br />
...
- 더 많은 사용자 노출
  ...
  - [SEO(Search Engine Optimization)](https://en.wikipedia.org/wiki/Search_engine_optimization)<br />
    ...
    이와 같은 측면에서 우리는 프로젝트 시작 지점에 여러 가지 기술들에 대해서 고려를 해야 하는 지점(활동)들이 있는데, 이 활동이 학습을 하기에 절호의 찬스(!)니까 이 시간을 아주 적극적으로 활용하는 것이 좋을 것이라 생각한다. 🙂
